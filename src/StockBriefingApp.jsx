  ...
  return (
    <div className="app">
      ...
      <nav className="tabs">
        ...
        <button className={`tab ${activeTab === 'stocks' ? 'active' : ''}`} onClick={() => setActiveTab('stocks')}>Manage Stocks</button>
        ...
        {activeTab === 'stocks' && (
          <section className="stocks-section">
            ...
            <form onSubmit={addStock} className="add-stock-form">
              <input type="text" placeholder="Enter stock ticker (e.g., AAPL)" value={newTicker} onChange={(e) => setNewTicker(e.target.value)} />
            ...
            <div className="stocks-list">
              {stocks.map(stock => (
                <div key={stock.ticker} className="stock-row">
                  ...
                  <button className="remove-btn" onClick={() => removeStock(stock.ticker)}>✕</button>
                </div>
              ))}
            </div>
          </section>
              ...
              {briefings.length === 0 ? (
                ...
                briefings.map((b, i) => (
                  <details key={i} className="briefing-item">
                    <summary>{new Date(b.timestamp).toLocaleString()}</summary>
                    <pre>{b.content}</pre>
                  </details>
                ))
              )}
